...
Original file is located at
    https://colab.research.google.com/drive/1Zqwp3X4OBX0zN4AsFF__XLz-Eqr1yDev
"""
...
plot_pytorch_data_stream(data_dogs_vs_cats['train'], max_images=6)
...
val_acc_fr1 = "Validation accuracy freezing last one layer"
val_acc_fr2 = "Validation accuracy freezing last two layers"
val_acc_fr3 = "Validation accuracy freezing half of layers"
df = pd.DataFrame(columns=[val_acc_ft, val_acc_fr1, val_acc_fr2, val_acc_fr3])
...
model, metrics = freeze_and_train2(data_dogs_vs_cats, MODEL_NAME, SETS, EPOCHS, NUM_GPUS, LR, 
                                  MOMENTUM, LR_STEP, LR_EPOCHS, verbose=False)
print(max(metrics['val_acc']).cpu().numpy())
...
model, metrics = freeze_and_train3(data_dogs_vs_cats, MODEL_NAME, SETS, EPOCHS, NUM_GPUS, LR, 
                                  MOMENTUM, LR_STEP, LR_EPOCHS, verbose=False)
print(max(metrics['val_acc']).cpu().numpy())
...
model, metrics = freeze_and_train4(data_dogs_vs_cats, MODEL_NAME, SETS, EPOCHS, NUM_GPUS, LR, 
                                  MOMENTUM, LR_STEP, LR_EPOCHS, verbose=False)
print(max(metrics['val_acc']).cpu().numpy())
